...
class _LoginState extends State<Login> with TickerProviderStateMixin {
  ...
  void initState() {
    currentPage = 0;
    print("ischeckownerordriver $ischeckownerordriver");
    _otp.clear();
    controller.text = '';
    proImageFile1 = null;
    gender = '';
...
//pick image from camera
    ...
    if (verify == true) {
      if (userDetails['uploaded_document'] == false) {
        Navigator.pushAndRemoveUntil(
            context, MaterialPageRoute(builder: (context) => const RequiredInformation()), (route) => false);
        ...
        if (userDetails['role'] != 'owner' && userDetails['enable_bidding'] == true) {
          Navigator.pushAndRemoveUntil(
              context, MaterialPageRoute(builder: (context) => const RidePage()), (route) => false);
        } else if (userDetails['role'] != 'owner' && userDetails['enable_bidding'] == false) {
          Navigator.pushAndRemoveUntil(
  ...
  registerDriver(mobile, country, name) async {
    print("calll");
    setState(() {
      loginLoading = true;
    });
    ...
    Map<String, dynamic> requestBody = {
      ...
      "mobile": mobile,
      "country": country,
      "service_location_id": "0be5cd70-291a-45bd-a451-a6f49345739d"
    };

    try {
      var response = await http.post(Uri.parse('${url}api/v1/driver/register'),
          headers: {"Content-Type": "application/json", "Accept": "application/json"}, body: jsonEncode(requestBody));
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          print("jsonVal['verification_code'] ${jsonVal['verification_code']}");
          otpSent = true;
          getotp = jsonVal['verification_code'];
          _error = '';
        });
      } else if (response.statusCode == 422) {
        debugPrint(response.body);
        String errorMsg = jsonVal['message'];
        if (jsonVal['errors'] != null && jsonVal['errors']['mobile'] != null) {
          errorMsg = jsonVal['errors']['mobile'][0];
        }
        Fluttertoast.showToast(
            msg: errorMsg,
            toastLength: Toast.LENGTH_SHORT,
  ...
  verifyOTP(String mobile, String otp) async {
    ...
    print("call");
    try {
      var response = await http.post(Uri.parse('${url}api/v1/driver/otpVerify'), body: {
        "mobile": mobile,
        "otp": otp,
      });

      print("call 2");
      debugPrint("driverLogin==== ${response.body}");
      bearerToken.clear();
      if (response.statusCode == 200) {
        ...
        bearerToken.add(BearerClass(
            type: responseData['token_type'].toString(),
        ...
        setState(() {
          ...
          pref.setString('Bearer', responseData['access_token']);
        });
  ...
  DriverLogin(mobile, country) async {
    setState(() {
      loginLoading = true;
    });
    dynamic result;
      ...
      child: Directionality(
          ...
          child: ValueListenableBuilder(
              ...
              builder: (context, value, child) {
                return ischeckownerordriver == "driver"
                    ? Container(
                        child: Stack(
                        children: [
                          SizedBox(
                                              ...
                                              .map((k, value) => MapEntry(
                                                    ...
                                                    Container(
                                                      margin: EdgeInsets.only(
                                                        right:
                                                            (k < loginImages.length - 1 || languageDirection == 'rtl')
                                                                ? media.width * 0.025
                                                                : 0,
                                                  ...
                                                  children: [
                                                    ...
                                                    Container(
                                                      ...
                                                      child: TextField(
                                                        ...
                                                        controller: _mobile,
                                                        decoration: InputDecoration(
                                                            hintText: languages[choosenLanguage]['text_mobile_number'],
                                                            border: InputBorder.none),
                                                      ),
                                                    ),
                                                    const SizedBox(height: 20),
                                                    otpSent == true
                                                        ...
                                                        : Container(),
                                                    otpSent == true ? const SizedBox(height: 15) : Container(),
                                                    if (_error != '')
                                                      Column(
                                                        children: [
                                                          Container(
                                                    ...
                                                    Button(
                                                      text: otpSent == true ? 'Confirm' : 'Verify Mobile',
                                                      onTap: () async {
                                                        if (_name.text.isEmpty) {
                                                          ...
                                                          setState(() {
                                                            _error = 'Please enter Mobie Number';
                                                          });
                                                      ...
                                                      child: Column(
                                                        children: [
                                                          ...
                                                          otpSent == true
                                                              ...
                                                              : Container(),
                                                          otpSent == true ? const SizedBox(height: 20) : Container(),
                                                          const SizedBox(height: 10),
                                                          if (_error != '')
                                                            Column(
                                                              children: [
                    ...
                    : Stack(
                        children: [
                                    ...
                                    child: SingleChildScrollView(
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          if ((withOtp == false || otpSent == true || signIn == 1) &&
                                              newPassword == false)
                                            Column(
                                              children: [
                                                SizedBox(
                                                    ...
                                                    children: [
                                                      Expanded(
                                                        child: TextField(
                                                          ...
                                                          obscureText: ((withOtp == false || signIn == 1) &&
                                                                  showPassword == false)
                                                              ? true
                                                              : false,
                                                        ),
                                          ...
                                          AnimatedCrossFade(
                                              ...
                                              crossFadeState:
                                                  (signIn == 1 && otpSent == true && mobileVerified == false)
                                                      ? CrossFadeState.showSecond
                                                      : CrossFadeState.showFirst,
                                              duration: const Duration(milliseconds: 200)),
                                                ...
                                                if (newPassword == true) {
                                                  if (_newPassword.text.length >= 8) {
                                                    var val = await updatePassword(
                                                        _email.text, _newPassword.text, isLoginemail);
                                                    if (val == true) {
                                                      withOtp = false;
                                                      otpSent = false;
                                                      _password.clear();
                                                  ...
                                                  if (withOtp == true) {
                                                    ...
                                                    if (_password.text.isNotEmpty &&
                                                      ...
                                                      _error = 'Invaild Mobile Number or Password';
                                                    }
                                                  }
                                                } else {
                                                  if (mobileVerified == true) {
                                                    ...
                                                    if (email != _email.text) {
                                                      val = await verifyUser(_mobile.text, 0, _password.text,
                                                          _email.text, withOtp, forgotPassword);
                                                    } else {
                                                      val = false;
                                                    }
                                                    if (val == false) {
                                                    ...
                                                    if (_name.text.isNotEmpty &&
                                                      ...
                                                      if (regExp.hasMatch(_mobile.text) &&
                                                        ...
                                                        if (regex.hasMatch(_email.text)) {
                                                          ...
                                                          var verify = await verifyUser(_mobile.text, 0, '',
                                                              _email.text, withOtp, forgotPassword);
                                                          if (verify == false) {
                                                            if (isCheckFireBaseOTP) {
                                                              var val = await otpCall();
                                                              ...
                                                              Future.delayed(const Duration(seconds: 1), () {
                                                                _scroll.position
                                                                    .moveTo(_scroll.position.maxScrollExtent);
                                                              });
                                                            } else {
                                                              var val = await sendOTPtoMobile(
                                                                  (signIn == 1) ? _mobile.text : _email.text,
                          ...
                          Positioned(
                              ...
                              left: (languageDirection == 'ltr') ? media.width * 0.05 : null,
                              right: (languageDirection == 'rtl') ? media.width * 0.05 : null,
                              child: InkWell(
                                onTap: () {
