...
class _RequiredInformationState extends State<RequiredInformation> {
  ...
  Future<void> fetchInitialDetails() async {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ischeckownerordriver = userDetails['role'].toString();
      setState(() {});
    });
  }
  Future init()async{
                      ...
                      onTap: () async {
                        if (profileCompleted != true && userDetails.isEmpty) {
                          var nav = await Navigator.push(
                              context, MaterialPageRoute(builder: (context) => ProfileInformation()));
                          if (nav != null) {
                            if (nav) {
                              setState(() {});
                            }
                    ...
                    InkWell(
                      onTap: () async {
                        if (profileCompleted == true) {
                          // carInformationCompleted == true && userDetails.isNotEmpty
                          // carInformationCompleted = false;
                          // if (carInformationCompleted != true && userDetails.isEmpty) {
                          if (carInformationCompleted == false) {
                            // ignore: prefer_typing_uninitialized_variables
                            var nav;
                            // if (ischeckownerordriver == 'driver') {
                            if (ischeckownerordriver == 'driver') {
                              isowner = false;
                              nav = await Navigator.push(context, MaterialPageRoute(builder: (context) => CarInformation(frompage: 1)));
                            } else {
                              nav = await Navigator.push(
                                  context, MaterialPageRoute(builder: (context) => const OwnersRegister()));
                            }
                            if (nav != null) {
                              if (nav) {
                                setState(() {});
                    ...
                    InkWell(
                      onTap: () async {
                        if (carInformationCompleted || userDetails.isNotEmpty) {
                          var nav =
                              await Navigator.push(context, MaterialPageRoute(builder: (context) => UploadDocument()));
                          if (nav != null) {
                            if (nav) {
                              setState(() {});
                            }
                      ...
                      child: Row(
                        children: [
                          ...
                          (documentCompleted == true ||
                                  (userDetails['uploaded_document'] == true && userDetails['declined_reason'] == null))
                              ? Image.asset(
                                  'assets/images/done_icon.png',
                                  height: media.width * 0.05,
                                  width: media.width * 0.05,
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child: (userDetails['declined_reason'] == null)
                                        ...
                                        : MyText(
                                            text: languages[choosenLanguage]['text_account_declined'],
                                            size: media.width * fourteen,
                                            fontweight: FontWeight.bold,
                                            color: whiteText,
                                          ),
