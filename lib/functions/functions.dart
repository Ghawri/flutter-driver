...
uploadDocs() async {
  ...
  try {
    ...
    debugPrint("docUpdate------- ${respon.body}");
...
verifyUser(String number, int login, String password, String email, isOtp,
  ...
  try {
    ...
    debugPrint("verifyUser==== ${response.body}");
    if (response.statusCode == 200) {
...
getUserDetails() async {
  ...
  try {
    ...
    userDetails.clear();
    debugPrint("response=-===== ${bearerToken[0].token}");
    if (response.statusCode == 200) {
...
getWalletHistory() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      dev.log(walletBalance.toString());
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ? geolocs.AndroidSettings(
        ...
        foregroundNotificationConfig:
          ...
          notificationText:
              "Buraaq Driver will continue to receive your location in background",
